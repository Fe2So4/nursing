      ...
      <div class="info-content-container">
        ...
        >
          ...
          >
            ...
            <tbody>
              <template v-if="basicEquipmentStr.length > 0">
                <tr
                  v-for="item in basicEquipmentStr"
                  :key="item.name"
                >
                  <td>
                    {{ item.name }}
    ...
    search () {
      ...
      this.$store.dispatch('ReqNursingDocumentJiChu', obj).then(res => {
        ...
        if (res.data.code === 200) {
          ...
          if (this.IsEmpty(wenshuData.basicEquipmentStr)) {
            this.basicEquipmentStr = JSON.parse(wenshuData.basicEquipmentStr).items
          }
